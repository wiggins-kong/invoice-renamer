...
- [x] 横向：命名模板「预设」在控件行内（保留原位置语义，`spacer` 推至行尾，`.preset-inline` 不拆散）——用户反馈放标题行右侧不合适，改回控件行末
- [x] 横向：分隔符组（标签+4按钮+其他输入+添加）包 `.sep-group` nowrap 整组不拆散
- [x] 横向：命名模板控件行改 `.row-ctl.nowrap` 不换行，字段下拉 `fieldSel` 收缩（180→min 110px）吸收空间；**窗口最小宽度 940→1080**（保证含预设的整行能一次放下）
- [x] 纵向：全局细滚动条（9px 圆角、主题色、悬停加深）；拖拽区紧凑化（图标 44→34、padding 34→20、标题 15→14）；chips/preview 微压缩
- [x] 验证：verify-layout.js 多尺寸截图（1180x820 / 1080x820）——控件行（含预设）整行、字段下拉正常、无重叠溢出
- **状态：** complete
